...
assert_contains "$ROOT_DIR/restore.sh" 'OPENVPN_RESTORE_PUBLIC_ENDPOINT'
assert_contains "$ROOT_DIR/restore.sh" 'confirm_restore_public_endpoint\('
assert_contains "$ROOT_DIR/restore.sh" 'refresh_client_common_remote\('
assert_contains "$ROOT_DIR/restore.sh" 'Detected public endpoint:'
assert_contains "$ROOT_DIR/restore.sh" 'Public IPv4 address or DNS name:'
assert_contains "$ROOT_DIR/restore.sh" 'is_dns_name\('
assert_contains "$ROOT_DIR/restore.sh" 'https://ifconfig\.co'
assert_contains "$ROOT_DIR/restore.sh" 'systemctl disable --now openvpn@server\.service'
assert_contains "$ROOT_DIR/restore.sh" 'systemctl disable --now openvpn-server@server\.service'
assert_contains "$ROOT_DIR/restore.sh" 'remote " public_endpoint " " udp_port'
assert_contains "$ROOT_DIR/restore.sh" '\$1 == "remote"'
assert_contains "$ROOT_DIR/restore.sh" 'updated_remote = 1'
assert_contains "$ROOT_DIR/restore.sh" 'if \(!updated_remote\) \{'
